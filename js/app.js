...
function syncDNReject(i){validateDNLine(i)}
function syncDNAccept(i){validateDNLine(i)}
function adjustDNQty(i,type,direction){
 ...
 const step=Number(qtyStep(it.uom))||1,current=Number(el.value)||0;
 el.value=roundQtyForUom(Math.max(0,current+(direction*step)),it.uom);validateDNLine(i);
}
