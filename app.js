...
async function saveDelivery(){
 ...
 const dnNo=await allocateDocumentNumber('deliveryNote',new Date(date+'T00:00:00'));const d={id:'DN-'+Date.now(),dnNo,date,status:'Out for Delivery',deliveryAccessToken:createDeliveryAccessToken(),deliveryAccessCreatedAt:new Date().toISOString(),vehicle:document.getElementById('dn-vehicle').value.trim(),remarks:document.getElementById('dn-remarks').value.trim(),items,dispatchedAt:new Date().toISOString(),dispatchedBy:deliveryActor(),created:new Date().toISOString()};so.deliveries=so.deliveries||[];so.deliveries.push(d);await saveSalesOrders();closeModal('so-delivery-modal');showToast(dnNo+' saved and dispatched','success');renderSOPage();renderDNPage();viewSO(so.id);
}
